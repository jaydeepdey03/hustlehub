...
const Milestones = (props: MilestoneProps) => {
  ...
  const UpdateMilestone = (milestone: string[]) => {
    ...
    if (id) {
      DB.updateDocument('646cfa393629aedbd58f', "646cfa7aa01148c42ebf", id, { milestones: milestone }).then(() => {
      ...
      <VStack textAlign="start" align="start" mb={5}>
        <Box zIndex={5} w="100%">
          ...
          <Box >
            ...
            {(user && startup && (user?.$id === startup?.cofounder || user?.$id === startup?.founder)) && <Box
              w="100%"
              h="50px"
              display="flex"
              justifyContent="center"
              alignItems="center"
            >
              <Center h="8" w="8" rounded="full" border={"1px dotted"} onClick={onOpen} cursor={"pointer"}>
                <Icon as={AddIcon} w={4} h={4} color={'gray.500'} />
              </Center>
              <Text color={'gray.500'} fontSize={"md"} fontWeight={"normal"} ml={2}>
                Add Milestones
              </Text>
            </Box>}
          </Box>
              ...
              {(formik) => (
                <form onSubmit={formik.handleSubmit}>
                  <FieldArray name="milestone">
                    {({ push, remove }) => (
                      <>
                        ...
                        <Button
                          mt={4} w="full" leftIcon={<AddIcon />} onClick={() => push('')}>
                          Add Milestone
                        </Button>
                      </>
                    )}
                  ...
                  <Button
                    ...
                    mt={4} w="full" type="submit" isLoading={updateMilestoneLoading}>
                    Update Milestone
                  </Button>
                </form>
              )}
