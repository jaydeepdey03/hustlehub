import { Button, FormControl, FormErrorMessage, FormHelperText, FormLabel, Input, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, NumberDecrementStepper, NumberIncrementStepper, NumberInput, NumberInputField, NumberInputStepper, Stack, Textarea, UseDisclosureReturn } from "@chakra-ui/react"
...
interface UpdateHackathonModalProps {
    ...
    UpdateHackathon: (title: string, description: string, image: File | string, noOfMembers: number, link: string, docId: string) => void,
    UpdateHackathonLoading: boolean
}

const UpdateHackathonModal = (props: UpdateHackathonModalProps) => {

    const { hackathon, updateHackathonDisclosure, UpdateHackathon, UpdateHackathonLoading } = props
    console.log(hackathon, 'hackathon from modal')
    return (
        <Modal onClose={updateHackathonDisclosure.onClose} size={"xl"} isOpen={updateHackathonDisclosure.isOpen} >
            <ModalOverlay />
            <ModalContent>
                            ...
                            <form onSubmit={formik.handleSubmit}>
                                <ModalBody>
                                    <Stack spacing={8} mx={'auto'} maxW={'lg'}>
                                        <Stack spacing={4}>
                                            <FormControl id="title" isInvalid={Boolean(!!formik.errors.title && formik.touched.title)}>
                                            ...
                                            <FormControl id="idea" isInvalid={Boolean(!!formik.errors.idea && formik.touched.idea)}>
                                                ...
                                                />
                                                <FormErrorMessage>{String(formik.errors.idea)}</FormErrorMessage>
                                            </FormControl>

                                            <FormControl id="link" isInvalid={Boolean(!!formik.errors.link && formik.touched.link)}>
                                                <FormLabel>Link</FormLabel>
                                                <Input type="text"
                                                    placeholder='Hackathon Link'
                                                    {...formik.getFieldProps('link')}
                                                />
                                                <FormErrorMessage>{String(formik.errors.link)}</FormErrorMessage>
                                            </FormControl>
                                            ...
                                            <FormControl>
                                                ...
                                                <NumberInput
                                                    ...
                                                    onChange={(value) => formik.setFieldValue('noOfMembers', value)}
                                                ...
                                                >
                                                    ...
                                                    <FormErrorMessage>{String(formik.errors.noOfMembers)}</FormErrorMessage>
                                                </NumberInput>
                                            </FormControl>
                                        </Stack>
                                    </Stack>
