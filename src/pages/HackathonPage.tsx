...
const HackathonPage = () => {
  ...
  useEffect(() => {
    if (id) {
      DB.getDocument("646cfa393629aedbd58f", "646ed5510d2cb68ff19f", id)
        .then((res) => {
      ...
      DB.getDocument("646cfa393629aedbd58f", "646ed5510d2cb68ff19f", id)
        .then((res) => {
          ...
          DB.updateDocument(
            ...
            id,
            {
              members: res.members.filter(
                (member: string) => member !== details.$id,
              ),
            },
          )
            .then(() => {
  ...
  console.log(
    selectedHackathon?.members?.includes(details?.$id) ||
      selectedHackathon?.creator === details?.$id,
    "selectedHackathon",
  );
      ...
      >
        ...
        >
          ...
          <Image
            ...
            mt="5"
            fallback={<Skeleton mt="4" height="200px" width="90%" />}
          />
          <Text
          ...
          <VStack spacing={"10"} width={"full"} alignItems={"center"}>
            <Box>
              <Profilecard
                userId={selectedHackathon.creator}
                created={true}
                details={true}
              />
            </Box>
        ...
        >
          ...
          <Box h="40%" maxW="full">
            <Text fontSize={"xs"} as="u" fontWeight={300}>
              Hackathon Idea
            </Text>
            <Box minH="10vh" overflowY="auto">
              <Text fontSize={"md"} fontWeight={300}>
                {selectedHackathon.idea}
              </Text>
            </Box>
          </Box>
          ...
          <Flex direction={"column"} p="6" pl="0">
            ...
            <Text as={ChakraLink} target="_blank" href={selectedHackathon.link}>
          ...
          {selectedHackathon?.members?.length === 0 && (
            <Text fontSize={"md"} fontWeight={300} textAlign={"center"}>
              No members yet
            </Text>
          )}
          <Box display="flex" justifyContent="center">
            <Grid
              templateColumns={`repeat(${
                selectedHackathon?.members?.length === 1 ? "1" : "2"
              }, 1fr)`}
              gap="9"
              justifyContent="center"
            >
