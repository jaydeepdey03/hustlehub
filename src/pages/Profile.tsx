...
function Profile() {
  ...
  const UpdateStartup = (
    ...
    storage
      .createFile("647a464fabf94fdc2ebf", ID.unique(), image as File)
      .then((res) => {
        const url = storage.getFilePreview("647a464fabf94fdc2ebf", res.$id);
        DB.updateDocument(
          "646cfa393629aedbd58f",
          "646cfa7aa01148c42ebf",
          docId,
          {
            title,
            idea: description,
      ...
      .then((res: AuthUser) => {
        ...
        DB.listDocuments("646cfa393629aedbd58f", "646cfa7aa01148c42ebf")
          .then((res1) => {
            ...
            const filteredStartups = startupDocuments.filter(
              (startup) => startup.cofounder === res?.$id,
            );
      ...
      .then((res: AuthUser) => {
        ...
        DB.listDocuments("646cfa393629aedbd58f", "646ed5510d2cb68ff19f")
          .then((res1) => {
            ...
            const filteredHackathon = hackathonDocument.filter((hackathon) =>
              hackathon.members.includes(res?.$id),
            );
      ...
      .then((res: AuthUser) => {
        ...
        DB.listDocuments("646cfa393629aedbd58f", "646cfa7aa01148c42ebf")
          .then((res1) => {
            ...
            // Filter the array based on cofounder property
            const filteredStartups = startupDocuments.filter(
              (startup) => startup.founder === res?.$id,
            );
      ...
      .then((res: AuthUser) => {
        ...
        DB.listDocuments("646cfa393629aedbd58f", "646ed5510d2cb68ff19f")
          .then((res1) => {
            ...
            const filteredStartups = hackathonDocument.filter(
              (hackathon) => hackathon.creator === res?.$id,
            );
  ...
  useEffect(() => {
    if (id) {
      DB.getDocument("646cfa393629aedbd58f", "646edbd8de898ccf87c5", id)
        .then((res) => {
  ...
  const deleteStartup = (startupId: string) => {
    DB.deleteDocument("646cfa393629aedbd58f", "646cfa7aa01148c42ebf", startupId)
      .then((res) => {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Avatar size="2xl" name={user.name} mt="5" />
          <Text
          ...
          >
            {details.twitterLink && (
              <ChakraLink href={details.twitterLink}>
                <Icon as={FiTwitter} w={5} h={5} />
              </ChakraLink>
            )}
            {details.linkedinLink && (
              <ChakraLink href={details.linkedinLink}>
            ...
            <TabPanels>
              <TabPanel>
                ...
                <VStack rounded="md" overflow="hidden" spacing={0}>
                  {userStartup.map(
                    (startup: StartupInterface, index: number) => (
                      <Fragment key={index}>
                        <Grid
                      ...
                      <Fragment key={index}>
                        ...
                        >
                          ...
                          >
                            <Text
                              as={Button}
                              onClick={updateHackathonDisclosure.onOpen}
                            >
                              Edit
                            </Text>
                            <Center
